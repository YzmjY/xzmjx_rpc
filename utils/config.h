 ...
 public:
  std::map<std::string, T> operator()(const std::string& f) {
    ...
    for (auto iter = node.begin(); iter != node.end(); iter++) {
      ss.str("");
      ss << iter->second;
      ans.insert(std::make_pair(iter->first.Scalar(),
 ...
 public:
  ...
  std::string toString() override {
    try {
      ...
      SPDLOG_ERROR(
          "ConfigVar::toString() exception {} ,convert: {} to string name =  "
          "{}",
          ex.what(), typeid(T).name(), m_name);
    }
    return "";
  }
  ...
  void fromString(const std::string& f) override {
    try {
      ...
      SPDLOG_ERROR(
          "ConfigVar::toString() exception {} ,convert: {} to string name =  "
          "{} - {}",
          ex.what(), typeid(T).name(), m_name, f);
    }
  }
...
class Config {
 public:
  ...
  static typename ConfigVar<T>::ptr Lookup(const std::string& name,
                                           ...
                                           const std::string& description) {
    {
      ...
      if (iter != GetConfigMap().end()) {
        ...
        if (p) {
          ...
          SPDLOG_ERROR(
              "Lookup name({}) exists but type not {},real type = {} {}", name,
              typeid(T).name(), iter->second->getTypeName(),
    ...
    typename ConfigVar<T>::ptr val =
        std::make_shared<ConfigVar<T>>(name, defaultVal, description);
  ...
  static typename ConfigVar<T>::ptr Lookup(const std::string& name) {
    ...
    if (p) {
      ...
      SPDLOG_ERROR("Lookup name({}) exists but type not {},real type = {} {}",
                   name, typeid(T).name(), iter->second->getTypeName(),
                   iter->second->toString());
      return nullptr;
    }
  }
