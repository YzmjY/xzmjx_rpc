  ...
  while (true) {
    ...
    switch (type) {
      case (uint8_t)Protocol::Type::kRpcHeartbeat:
        SPDLOG_LOGGER_DEBUG(g_logger, "heartbeat to service provider:{}",
                            session->getPeerAddressString());
        rsp = handleHeartbeat();
        break;
...
std::string RpcServiceRegistry::handleProvider(RpcSession::ptr session,
                                               Protocol::ptr req) {
  uint32_t port = 0;
  ...
  IPv4Address::ptr addr =
      std::dynamic_pointer_cast<IPv4Address>(session->getPeerAddress());
  addr->setPort(port);
...
void RpcServiceRegistry::unregisterService(const std::string& server_addr) {
  ...
  if (range.first == range.second) {
    ...
    for (auto i : del) {
      SPDLOG_LOGGER_DEBUG(g_logger,
                          "unregister service : [{}] from server : [{}]",
                          i->first, i->second);
...
Protocol::ptr RpcServiceRegistry::handleRegisterService(const std::string& addr,
                                                        Protocol::ptr req) {
  ...
  do {
    ...
    }
    {
      ...
      SPDLOG_LOGGER_DEBUG(g_logger,
                          "register service : [{}] from server : [{}]", service,
                          addr);
...
Protocol::ptr RpcServiceRegistry::handleServiceDiscover(Protocol::ptr req) {
  ...
  do {
    ...
    if (range.first == range.second) {
      SPDLOG_LOGGER_WARN(g_logger, "service discover with unregister name:{}",
                         name);
      ctx.setCode(RPCErrorCode::kRpcNoMethod);
  ...
  serializer << ret;
  serializer.reset();
  return Protocol::Create(Protocol::Type::kRpcServiceDiscoverResponse,
                          std::move(ctx), serializer.toString(),
                          req->getSeqId());
}
} // namespace xzmjx::rpc