...
xzmjx::ConfigVar<uint64_t>::ptr g_client_alive_time = Config::Lookup(
    "rpc_server.client_alive_time", (uint64_t)(40), "rpc client alive time");
xzmjx::ConfigVar<uint64_t>::ptr g_connect_registry_timeout =
    Config::Lookup("rpc_server.connect_registry_timeout", (uint64_t)(5),
                   "rpc server connect to registry timeout");
xzmjx::ConfigVar<uint64_t>::ptr g_registry_heartbeat =
    Config::Lookup("rpc_server.registry_heartbeat_timeout", (uint64_t)(40),
                   "rpc registry heartbeat timeout");
...
bool RpcServer::bind(Address::ptr addr) {
  ...
  SPDLOG_LOGGER_DEBUG(g_logger, "server provider service with port :{}",
                      m_port);
...
bool RpcServer::bindRegistry(Address::ptr addr) {
  ...
  serializer << m_port;
  serializer.reset();
  Protocol::ptr provider = Protocol::Create(Protocol::Type::kRpcServiceProvider,
                                            serializer.toString());
...
void RpcServer::registerServiceToRegistry(const std::string& service) {
  ...
  serializer << service;
  serializer.reset();
  Protocol::ptr req = Protocol::Create(
      Protocol::Type::kRpcServiceRegisterRequest, serializer.toString());
...
bool RpcServer::start() {
  ...
  if (m_registry_session && m_registry_session->isConnected()) {
    ...
    registry_heartbeat_cb = [this]() {
      ...
      m_register_timer_id = m_register_timer.ExpireAt(
          std::chrono::seconds(g_registry_heartbeat->getValue()),
          registry_heartbeat_cb);
      m_registry_heartbeat_rsp = false;
    };
    m_register_timer_id = m_register_timer.ExpireAt(
        std::chrono::seconds(g_registry_heartbeat->getValue()),
    ...
    go[this]() { handleRegistrySessionRecv(); };

    go[this]() { handleRegistrySessionSend(); };
  }
...
void RpcServer::handleClient(Socket::ptr client) {
  ...
  auto on_timeout = [client]() {
    SPDLOG_LOGGER_WARN(g_logger, "client: {} alive timeout",
                       client->toString());
    client->close();
  };
  co_timer_id time_id = m_client_timer.ExpireAt(
      std::chrono::seconds(m_client_alive_time), on_timeout);
  while (client->isConnected()) {
    ...
    /// 重设心跳定时器
    time_id.StopTimer();
    time_id = m_client_timer.ExpireAt(std::chrono::seconds(m_client_alive_time),
                                      on_timeout);
...
Protocol::ptr RpcServer::handleMethodCall(Protocol::ptr req) {
  ...
  Serializer res;
  Context ctx;
  SPDLOG_LOGGER_DEBUG(g_logger, "receive a function call, name = {}",
                      func_name);
