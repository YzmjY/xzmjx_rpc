...
public:
    ...
    using FuncType = std::function<std::pair<Serializer,Context>(const std::string& name,const std::string& args)>;
    ...
    void registerMethod(const std::string& name,Func f) {
        FuncType real_call = [this,f](const std::string& name ,const std::string& args)->std::pair<Serializer,Context>{
...
private:
    template<class Func>
    std::pair<Serializer,Context> internal_method_impl(Func f, const std::string& name,const std::string& args) {
