...
namespace xzmjx::rpc{
Protocol::Protocol() :m_carry_msg(false)
                     , m_magic(kMagic)
...
Protocol::ptr Protocol::Create(Protocol::Type type, const std::string &content, uint64_t id) {
    Context ctx;
    ctx.setCode(RPCErrorCode::kRpcSuccess);
    return Create(type,std::move(ctx),content,id);
}

Protocol::ptr Protocol::Create(Protocol::Type type,Context ctx, const std::string &content, uint64_t id) {
