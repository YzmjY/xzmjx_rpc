...
Protocol::ptr RpcSession::recvProtocol() {
    ...
    if(n==0) {
        SPDLOG_LOGGER_INFO(g_logger,"socket closed,peer address = {}",getPeerAddressString());
        return nullptr;
    }
    if(n<0) {
        SPDLOG_LOGGER_ERROR(g_logger,"cannot read protocol header, need least {} bytes",Protocol::kHeaderLen);
        return nullptr;
    }
    ...
    if(msg->getMagic() != Protocol::kMagic) {
        SPDLOG_LOGGER_WARN(g_logger,"protocol magic not match {}",Protocol::kMagic);
        return nullptr;
    }
    if(msg->getVersion() != Protocol::kDefaultVersion) {
        SPDLOG_LOGGER_WARN(g_logger,"protocol version(version:{}) unsupported",msg->getVersion());
        return nullptr;
    }
