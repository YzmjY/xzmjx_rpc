...
public:
  ...
  std::pair<std::optional<T>, Context> call(const std::string& func_name,
                                            Types... args) {
  ...
  std::pair<std::nullopt_t, Context> call(const std::string& func_name,
                                          Types... args) {
    std::tuple<std::decay_t<Types>...> args_tuple{args...};
    Serializer s;
    s << func_name << args_tuple;
...
private:
  ...
  template <class T>
  std::pair<std::optional<T>, Context> call_internal(const std::string& name,
                                                     Serializer s) {
    Context ctx;
    ...
    /// 发送调用请求
    Protocol::ptr req =
        Protocol::Create(Protocol::Type::kRpcMethodRequest, s.toString(), id);
    SPDLOG_LOGGER_INFO(log::GetDefault(), "call method {} ,protocol:{}", name,
                       req->debugString());
    m_chan << req;
  ...
  template <VoidType T>
  std::pair<std::nullopt_t, Context> call_internal(const std::string& name,
                                                   Serializer s) {
    Context ctx;
    if (m_session == nullptr || m_is_closed) {
      ctx.setCode(RPCErrorCode::kRpcServerClosed);
