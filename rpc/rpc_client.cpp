...
namespace xzmjx::rpc {
xzmjx::ConfigVar<uint64_t>::ptr g_channel_size = Config::Lookup("rpc_client.channel_size",(uint64_t)(200),"rpc client channel size");
xzmjx::ConfigVar<uint64_t>::ptr g_connect_timeout = Config::Lookup("rpc_client.connect_timeout",(uint64_t)(10),"rpc client connect timeout");
xzmjx::ConfigVar<uint64_t>::ptr g_heartbeat_timeout = Config::Lookup("rpc_client.heartbeat_timeout",(uint64_t)(30),"rpc client heartbeat timeout");
xzmjx::ConfigVar<uint64_t>::ptr g_method_call_timeout = Config::Lookup("rpc_client.method_call_timeout",(uint64_t)(5),"rpc client method call timeout");

static auto g_logger = log::GetDefault();
RpcClient::RpcClient(co::Scheduler *worker): m_chan(g_channel_size->getValue())
...
bool RpcClient::connect(Address::ptr address) {
    ...
    m_heartbeat_cb = [this]() {
        ...
        m_heartbeat_timer_id = m_heartbeat_timer.ExpireAt(std::chrono::seconds(m_heartbeat_timeout),m_heartbeat_cb);
    };
...
void RpcClient::handleSend() {
    ...
    while(m_chan.pop(msg)) {
        if(false == m_session->sendProtocol(msg)) {
            SPDLOG_LOGGER_ERROR(g_logger,"send protocol failed,debug string = {}",msg->debugString());
        }
    }
}
...
void RpcClient::handleHeartbeat() {
    SPDLOG_LOGGER_INFO(g_logger,"receive server heartbeat,server:{}",m_session->getPeerAddressString());
    m_is_heartbeat_response = true;
}
