...
bool Socket::getOpt(int level,int optname,void* optval,socklen_t* optlen){
    ...
    if(rt != 0){
        SPDLOG_LOGGER_ERROR(g_logger,"getOpt sock={} level={} optname={} errno={} errstr",
...
bool Socket::setOpt(int level,int optname,const void* optval,socklen_t optlen){
    ...
    if(rt != 0){
        SPDLOG_LOGGER_ERROR(g_logger,"setOpt sock={} level={} optname={} errno={} errstr",
                            m_sock_fd,level,optname,errno, strerror(errno));
        return false;
    }
    return true;
...
Socket::ptr Socket::accept(){
    ...
    if(client_sock == -1){
        SPDLOG_LOGGER_ERROR(g_logger,"accept({}) errno={} errstr={}",
...
bool Socket::bind(const Address::ptr addr){
    ...
    if(::bind(m_sock_fd,addr->getAddr(),addr->getAddrLen())){
        SPDLOG_LOGGER_ERROR(g_logger,"bind():sock_fd={} address={} errno={} errstr={}",
...
bool Socket::listen(int backlog){
    ...
    if(::listen(m_sock_fd,backlog)){
        SPDLOG_LOGGER_ERROR(g_logger,"listen error errno={} errstr={}",
...
bool Socket::connect(const Address::ptr addr,uint64_t timeout_ms){
    ...
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock_fd,addr->getAddr(),addr->getAddrLen())) {
            SPDLOG_LOGGER_ERROR(g_logger,"connect() sock={} address={} errno={} errstr={}",
                                m_sock_fd,addr->toString(),errno, strerror(errno));
            close();
            return false;
        }
...
std::ostream& Socket::dump(std::ostream& os) const{
    os<<"[socket sock="<<m_sock_fd
      <<" is_connect="<<m_is_connect
      <<" family="<<m_family
      <<" type="<<m_type
      <<" protocol="<<m_protocol;
    if(m_local_address){
