...
TcpServer::TcpServer(co::Scheduler* worker,
                     co::Scheduler* io_worker,
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
    ...
    for(auto& addr:addrs){
        ...
        if(!sock->bind(addr)){
            SPDLOG_LOGGER_ERROR(g_logger,"bind failed errno={} errstr={} addr=[{}]",errno, strerror(errno),addr->toString());
            fails.push_back(addr);
            continue;
        }
        if(!sock->listen()){
            SPDLOG_LOGGER_ERROR(g_logger,"listen failed errno={} errstr={} addr=[{}]",errno, strerror(errno),addr->toString());
            fails.push_back(addr);
            continue;
        }
    ...
    for(auto& i:m_listen_socks){
        SPDLOG_LOGGER_INFO(g_logger,"type={} name={} server bind success:{} ",m_type,m_name,i->toString());
    }
    return true;
}
...
bool TcpServer::start(){
    ...
    for(auto& sock: m_listen_socks){
        ...
        go co_scheduler(m_accept_worker) std::bind(&TcpServer::startAccept,shared_from_this(),sock);
    }
...
void TcpServer::startAccept(Socket::ptr sock){
    while(!m_is_stop){
        ...
        if(client){
            ...
            go co_scheduler(m_io_worker) std::bind(&TcpServer::handleClient,shared_from_this(),client);
        }else{
            SPDLOG_LOGGER_ERROR(g_logger,"accept errno={} errstr={}",errno, strerror(errno));
        }
    }
}
}
